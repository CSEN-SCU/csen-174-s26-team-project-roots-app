...

{
  ...
  "extracted": {
    ...
    "detectedHours": string | undefined,   // Business hours if mentioned, e.g. "Open daily 8:00 AM – sunset"
    "instructions": string[] | undefined   // 2-4 practical tips or logistics from the content
  },
  "roadmap": {
    "kind": "route" | "project",
    ...
    "stops": [                 // ONLY for kind="route"
      {
        ...
        "travelMinutesFromPrev": number | undefined,
        "travelMode": "walk" | "drive" | "transit" | "bike" | undefined,
        "note": string | undefined   // Brief practical note
      }
    ],
    "steps": [                 // ONLY for kind="project"
      {
        ...
        "materials": string[] | undefined  // Specific materials with quantities
      }
    ]
  }
}
...
Extract a complete plan from this social media content and return it as raw JSON matching the schema in your instructions.`;
    ...
    if (extracted.roadmap?.stops?.length) {
      extracted.roadmap.stops = await Promise.all(
        extracted.roadmap.stops.map((s: Stop) => geocodeStop(s))
      );
    }
